...
router.get('/', [
    ...
    (0, express_validator_1.query)('sortBy').optional().isIn(['name', 'price', 'createdAt']),
    ...
    try {
        ...
        const { page = 1, limit = 10, search = '', category, status, inStock, sortBy = 'createdAt', sortOrder = 'desc' } = req.query;
        // Build filter object
        const filter = {};
        if (search) {
            filter.$or = [
...
router.post('/', [
    ...
    (0, express_validator_1.body)('category').isIn(['electronics', 'clothing', 'books', 'home', 'sports', 'beauty']).withMessage('Invalid category'),
...
router.put('/:id', [
    ...
    (0, express_validator_1.body)('status').optional().isIn(['active', 'inactive', 'out_of_stock', 'discontinued'])
], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
        ...
        const updatedProduct = await Product_1.Product.findByIdAndUpdate(req.params.id, { ...req.body }, { new: true, runValidators: true });
        if (!updatedProduct) {
            return res.status(404).json({ error: 'Product not found' });
        }
        // Transform MongoDB _id to id for client compatibility
